...
module.exports = app =>{
    ...
    const Hero = monggose.model('Hero')
    ...
    router.get('/heroes/list',async (req,res) => {
        const A = await Hero.find();
        A.unshift({
            name:'热门',
            heroList:await Hero.find().limit(159).lean()
        })
        const arr = [{},{},{},{},{},{},{}];
        function fn(){
            var a = 0;
            ...
            return function(){
                for(var i = 0;i<arr.length;i++){
                    var list = ['热门','战士','法师','坦克','刺客','射手','辅助']
                    arr[i].name = list[i],
                    arr[i].heroList = A[0].heroList.slice(a,b);
                    if(i==0){
                        a+=10;
                        b+=33;
                        console.log(i)
    ...
    router.get('/articles/:id',async (req,res) => {
        const data = await Article.find().where({_id:req.params.id});
